...
async function seed() {
  ...
  const [done, inProgress, toDo] = await db.insert(status).values([
    { title: "To-Do", column: "A" },
    { title: "In Progress", column: "B" },
    { title: "Done", column: "C" }
  ]).returning();
  const [low, medium, high] = await db.insert(priority).values([{ title: "Low" }, { title: "Medium" }, { title: "High" }]).returning();
  const currentDay = (0, import_dayjs.default)(/* @__PURE__ */ new Date());
