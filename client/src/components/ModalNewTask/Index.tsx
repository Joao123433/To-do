...
export function ModalNewTask({ isOpen, onRequestClose }: ModalTaskInterface) {
    ...
    >
      ...
      <form action="" className="flex flex-col gap-5 pl-2">
        <div className="flex flex-row gap-2 items-center justify-between">
          ...
          <select className='w-input-modal bg-gray-800 py-1 px-2 rounded hover:bg-engengrau duration-300' value={selectPriority} onChange={(e) => setSelectPriority(e.target.value)}>
        ...
        <div className="flex flex-row gap-2 items-center justify-between">
          ...
          <input type="date" className="w-input-modal py-1 px-2 rounded hover:bg-engengrau duration-300" value={deadline} onChange={(e) => setDeadline(e.target.value)} required/>
        </div> 
        ...
        <div className="flex flex-row gap-2 items-center justify-between">
          ...
          <select className='w-input-modal bg-gray-800 py-1 px-2 rounded hover:bg-engengrau duration-300' value={selectStatus} onChange={(e) => setSelectStatus(e.target.value)}>
            <option>&nbsp;</option>
            {status.map(element => (
        ...
        <div className="flex flex-row gap-2 items-baseline justify-between">
          ...
          <textarea rows={4} className='w-input-modal bg-gray-800 py-1 px-2 rounded hover:bg-engengrau duration-300 resize-none' onChange={(e) => setComment(e.target.value)}>{comment}</textarea>
        </div> 

        <div className="flex flex-row justify-end gap-5">
          <button type="submit" className="font-bold px-3 py-2 rounded w-1/4 hover:brightness-75 duration-300" onClick={handleClick}>Salvar</button>
        </div>
      </form>
    </Modal>
  )
